"""Integration: inbound webhook receiver starts a workflow run with the POST body
as input; an unknown token is rejected."""
...
async def test_inbound_webhook_starts_run_with_body(admin_session: AsyncSession) -> None:
    ...
    # The receiver resolves the token, downgrades tenant, and seeds a run.
    result = await trigger_from_inbound(admin_session, "tok_ABC", {"score": 21})
    assert result is not None
    run_id = uuid.UUID(result["run_id"])
    await admin_session.commit()

    # Drive the seeded run: the script task read the inbound body.
    run = await WorkflowRunRepository(admin_session, org.id).get_by_id(run_id)
...
async def test_inbound_unknown_token_returns_none(admin_session: AsyncSession) -> None:
    ...
    assert await trigger_from_inbound(admin_session, "not-a-real-token", {"x": 1}) is None
...
async def test_inbound_disabled_endpoint_returns_none(admin_session: AsyncSession) -> None:
    ...
    await admin_session.commit()
    assert await trigger_from_inbound(admin_session, "tok_OFF", {}) is None
