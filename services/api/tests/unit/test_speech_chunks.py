...
ANSWER = (
    "The Magellan has 10-12 crew and is an Atlas Class Carrier. The Cassini has 9-11 crew. "
    "The Phoenix has 6-7 crew and serves as the fleet's only battleship. The Galileo is a "
...
class TestFirstChunkLeavesEarly:
    ...
    def test_opening_clause_is_emitted_before_the_answer_finishes(self) -> None:
        chunker = SentenceChunker()
        first = chunker.push("The Magellan has 10-12 crew, ")
        assert first == ["The Magellan has 10-12 crew,"]
...
class TestNeverStallsForPunctuation:
    def test_a_long_unpunctuated_run_is_still_spoken(self) -> None:
        ...
        chunks = chunker.push("Magellan Cassini Phoenix Odyssey Galileo Falcon " * 4)
...
class TestNonEnglish:
    def test_spanish_and_cjk_sentence_ends_are_boundaries(self) -> None:
        ...
        text = "La nave más pequeña es el Galileo. Tiene una tripulación de 5 a 6 miembros. "
        chunks = _run(text, 7)
        assert len(chunks) >= 2
...
class TestTheBufferEndIsNotABoundary:
    ...
    clause break. Caught live — the robot said "The Magellan has ten dash" and then "twelve
    ...
    def test_a_dash_range_is_never_split(self) -> None:
        ...
        assert chunker.push("The Magellan has 10–") == []
        out = chunker.push("12 crew. The Cassini has 9–11. ")
        tail = chunker.flush()
    ...
    def test_the_words_still_survive_the_stricter_rule(self) -> None:
        text = "Crew is 10–12 on the Magellan, 9–11 on the Cassini, and 6–8 on the Odyssey. That is all."
