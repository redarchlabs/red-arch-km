...
_WORK_ORDER_GUIDANCE = (
    ...
    "Plan only what you can actually do. Every step must be completable with the tools "
    "you have been given or by delegating to someone who has more — writing down a step "
    "that needs a capability nobody here holds does not create it, it just guarantees "
    "the order stalls on that step. If your web tool fetches one page, plan to read the "
    "pages that matter, not to crawl a site. Match the plan to the size of the request: "
    "'tell me what you think' is answered by looking and saying what you think, and a "
    "plan that has become a construction project has stopped being about the request. "
    "Where something genuinely cannot be checked here, say so in your answer — a partial "
    "answer that names its own gaps is worth far more than a complete plan that stalls.\n"
