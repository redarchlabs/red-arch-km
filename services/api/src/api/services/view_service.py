...
class ViewService:
    ...
    async def _resolve_own_record_id(
        ...
        *,
        ...
        (matched by ``field_slug``/``email``) for the ``record_id=me`` sentinel.

        Delegates to the shared :func:`resolve_own_record_id` so the view binding and
        the record-list ``@me`` filter apply the exact same identity rule.
        """
        return await resolve_own_record_id(
            self._session, self._org_id, entity_definition_id, email, field_slug=field_slug
        )
