...
class AgentRepository:
    ...
    async def list_all(self) -> list[Agent]:
        result = await self._session.execute(select(Agent).where(Agent.org_id == self._org_id).order_by(Agent.name))
    ...
    async def get(self, agent_id: uuid.UUID) -> Agent | None:
        result = await self._session.execute(select(Agent).where(Agent.id == agent_id, Agent.org_id == self._org_id))
        return result.scalar_one_or_none()

    async def get_by_name(self, name: str) -> Agent | None:
        result = await self._session.execute(select(Agent).where(Agent.name == name, Agent.org_id == self._org_id))
        return result.scalar_one_or_none()
