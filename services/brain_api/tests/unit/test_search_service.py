...
class TestDocumentExpansion:
    ...
    def test_top_document_siblings_are_added_in_reading_order(
        ...
        mock_stores.vector.list_document_chunks.return_value = [
            self._sibling(1, doc_key="fleet", chunk_order=1, text="Meridian, Kestrel, Halcyon."),
            self._sibling(2, doc_key="fleet", chunk_order=2, text="Corvair, Solaris, Sierra."),
        ]
        with patch("brain_api.openai_client.OpenAI"):
            service = SearchService(mock_stores, fake_settings)
        ...
        assert "Meridian" in result["hits"][1]["payload"]["text"]
...
class TestRerank:
    ...
    def dense_hits(self) -> list[SearchResult]:
        return [
            ...
            self._result("kestrel", 0.667, doc_key="fleet", text="USS Kestrel — Standard Crew Complement: 450"),
        ]
    ...
    def test_reranked_order_replaces_dense_order(
        ...
        assert [h["id"] for h in hits] == ["kestrel", "intro"]
